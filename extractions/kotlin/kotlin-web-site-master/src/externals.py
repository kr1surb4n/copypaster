        ...
        for it in item['replace']:
            ...
            def process(it):
                ...
                try:
                    ...
                    raise Exception(
                        "Failed to parse regex %s for %s." % (re_text, context)
                    ) from e
...
class ExternalMount:
    def __init__(self, build_mode, external_spec):
        ...
        self.inline: bool = (
            bool(external_spec['inline'] or 'False')
            if 'inline' in external_spec
            else False
        )
        self.wrap_code_snippets: bool = (
            bool(external_spec['wrap_code_snippets'] or 'False')
            if 'wrap_code_snippets' in external_spec
            else False
        )
        ...
        if self.external_base.startswith("/docs/reference/"):
            ...
            raise Exception(
                "Unknown external path %s in %s"
                % (self.external_path, self.external_base)
            )

        assert_valid_git_hub_url(
            self.external_repo, 'EXTERNAL MODULE: %s' % self.external_path
        )

        self.target_external_path = path.join(
            root_folder, 'pages', self.external_base.lstrip("/")
        )
        self.source_external_path = path.join(
            root_folder, 'external', self.external_path.lstrip("/")
        )
        # assuming external folder contains only sub repositories
        self.source_checkout_root = path.join(
            root_folder, 'external', self.external_path.split("/")[0]
        )

        self.nav_file = path.join(
            self.source_external_path, self.external_nav.lstrip("/")
        )
        self.replacements = _parse_replacements(external_spec, self.external_nav)
        self.explicit_github_edit_page = (
            external_spec['github_edit_page']
            if 'github_edit_page' in external_spec
            else None
        )
...
class ExternalItem:
    def __init__(self, module: ExternalMount, url_mappers, item):
        ...
        assert self.md.endswith(".md"), (
            "md path " + self.md + " must have `.md` extension"
        )
        assert self.url.endswith(".html"), (
            "url path " + self.url + "must have `.html` "
...
def _process_external_entry(self: ExternalMount, url_mappers, entry: dict) -> list:
    ...
    if self.wrap_code_snippets:
        ...
        def handle_match(m):
            if m.group(1).lower() == 'kotlin':
                ...
                return (
                    '\n\n<div class="sample" markdown="1" theme="idea" mode="'
                    + m.group(1)
                    + '">'
                    + m.group(0)
                    + '\n</div>\n\n'
    ...
    return [{'url': item.html, 'title': item.title}]
        ...
        def handle_match(m):
            ...
            if url in known_urls:
                ...
                if path.isfile(real_path):
                    is_image = (
                        real_path.endswith(ext)
                        for ext in [".jpg", ".jpeg", ".png", ".svg"]
                    )

                    if any(is_image):
                        image_relative_path = path.join(
                            mount.external_base.lstrip("/"), url
                        )
                        image_output_path = path.join(
                            root_folder, "assets", "externals", image_relative_path
                        )
                        ...
                        url = (
                            "{{ url_for(\"asset\", path=\""
                            + path.join("externals", image_relative_path)
                            + "\") }}"
                        )
                    else:
...
def _process_external_key(build_mode, data) -> list:
    ...
    if not _rant_if_external_nav_is_not_found(mount):
        # TODO: side effect
        data['content'] = [
            {'url': '/', 'title': 'external "%s" is it included' % mount.external_path}
        ]
        return [data]
...
def process_nav_includes(build_mode, data):
    if isinstance(data, list):
        children = [process_nav_includes(build_mode, item) for item in data]
    ...
    if isinstance(data, dict):
        return {key: process_nav_includes(build_mode, item) for key, item in data.items()}
