...
for event in events:
    ...
    for city in cities_array:
        ...
        if (
            abs(coordinates['lat'] - float(geocoded_coordinates['lat'])) < 0.1
            and abs(coordinates['lng'] - float(geocoded_coordinates['lng'])) < 0.1
        ):
            is_same = None
            while is_same is None:
                user_input = input(
                    "Are " + location + " and " + city['name'] + " the same place? (y/n)"
                )
            ...
            if is_same:
                ...
                events_file_content = events_file_content.replace(
                    old_location, new_location
                )
                with codecs.open(
                    path.join(data_folder, "events.xml"), 'w', encoding='utf-8'
                ) as events_file:
        ...
        print(
            ...
            + geocoded_coordinates['lat']
            + ","
            + geocoded_coordinates['lng']
            + ",15z"
        )
        city = {
            'name': location,
